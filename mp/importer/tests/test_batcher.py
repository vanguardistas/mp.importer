...
class TestRandom(TestCase):
    ...
    def create_test_seed(self):
        import sys
        import random
        #seed = 1							# create random seed from this value (generated by the system) TODO
        seed = random.SystemRandom()
        return seed

    def test_random_sampler_percentage(self):
        from .. import batcher
        source = 'abcdefghil'						# TODO should I use argument parser here and pass more arguments if random!? 
        seed = self.create_test_seed()				         
        print ("the seed to obtain these batches is", seed)		# TODO log seed value
        percentage = 50
        result = batcher.random_sampler(source, seed, percentage)	# generator that yields the values if satisfy the condition
        log=[]
        for element in result:              
            log.append(element)
        log_list = ''.join(log)
        print ("result list is", log_list)
    ...
    def test_random_sampler_elements(self):
        ...
        source = 'abcdefghil'						
        seed = 1				         
        print ("the seed to obtain these batches is", seed)
        percentage = 50
        result = batcher.random_sampler(source, seed, percentage)	
        log=[]
        for element in result:              
            log.append(element)
        log_list = ''.join(log)
        self.assertEqual(log_list, 'adefi')				# check the elements (knowing the seed)




# for the test, I pass seed value (so I know the result)
# in the reality, seed is generated within the script
